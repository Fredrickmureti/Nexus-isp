...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Syncing IP pools for router: ${routerId}`);
    ...
    const apiUrl = `${router.ip_address}${router.api_endpoint || '/rest'}/ip/pool`;
    const auth = btoa(`${router.username}:${router.password}`);

    console.log(`Fetching IP pools from: ${apiUrl}`);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        count: pools.length,
        message: `Synced ${pools.length} IP pools`,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    console.error('Error syncing IP pools:', error);
    return new Response(
